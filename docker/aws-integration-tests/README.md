...
cargo test -p inferadb-engine-config --features aws-secrets --lib aws
...
1. Write tests in `crates/inferadb-engine-config/src/secrets/aws.rs`
...
4. Iterate in interactive mode:
   ...
   ./docker/aws-integration-tests/shell.sh
   # Inside container
   cargo test -p inferadb-engine-config --features aws-secrets <test_name>
   ```
