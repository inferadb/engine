...
impl TailscaleServiceDiscovery {
    ...
    ///
    /// ```rust
    /// use inferadb_engine_discovery::tailscale::{TailscaleServiceDiscovery, RemoteClusterConfig};
    ///
...
mod tests {
    ...
    fn test_parse_tailscale_url() {
        let url = "http://inferadb-engine.eu-west-1.ts.net:8080";
        let (service, domain, port, scheme) = parse_tailscale_url(url).unwrap();

        assert_eq!(service, "inferadb-engine");
    ...
    fn test_remote_cluster_config() {
        let config = RemoteClusterConfig {
            ...
            service_name: "inferadb-control".to_string(),
            port: 3000,
        };
