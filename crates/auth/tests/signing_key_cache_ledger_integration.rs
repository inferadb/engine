...
struct FailableStore {
    ...
    failure: std::sync::RwLock<Option<inferadb_common_storage::StorageError>>,
}
...
impl FailableStore {
    ...
    fn set_failure(&self, failure: Option<inferadb_common_storage::StorageError>) {
        *self.failure.write().expect("lock") = failure;
    }

    /// Reconstruct error since StorageError doesn't implement Clone
    fn reconstruct_error(
        error: &inferadb_common_storage::StorageError,
    ) -> inferadb_common_storage::StorageError {
        match error {
            inferadb_common_storage::StorageError::Connection { message, .. } => {
                inferadb_common_storage::StorageError::connection(message)
            },
            inferadb_common_storage::StorageError::Timeout => {
                inferadb_common_storage::StorageError::timeout()
            },
            inferadb_common_storage::StorageError::NotFound { key, .. } => {
                inferadb_common_storage::StorageError::not_found(key)
            },
            inferadb_common_storage::StorageError::Internal { message, .. } => {
                inferadb_common_storage::StorageError::internal(message)
            },
            _ => inferadb_common_storage::StorageError::internal("unknown error type"),
        }
    }
}
...
impl PublicSigningKeyStore for FailableStore {
    ...
    ) -> Result<(), inferadb_common_storage::StorageError> {
    ...
    async fn get_key(
        ...
        namespace_id: i64,
        kid: &str,
    ) -> Result<Option<PublicSigningKey>, inferadb_common_storage::StorageError> {
        if let Some(ref err) = *self.failure.read().expect("lock") {
            return Err(Self::reconstruct_error(err));
        }
    ...
    async fn list_active_keys(
        &self,
        namespace_id: i64,
    ) -> Result<Vec<PublicSigningKey>, inferadb_common_storage::StorageError> {
        if let Some(ref err) = *self.failure.read().expect("lock") {
            return Err(Self::reconstruct_error(err));
        }
...
async fn test_ledger_unavailability_fallback_integration() {
    ...
    failable_store.set_failure(Some(inferadb_common_storage::StorageError::connection(
...
async fn test_ledger_timeout_fallback_integration() {
    ...
    // Simulate timeout
    failable_store.set_failure(Some(inferadb_common_storage::StorageError::timeout()));
...
async fn test_non_transient_error_no_fallback_integration() {
    ...
    failable_store
        .set_failure(Some(inferadb_common_storage::StorageError::internal("database corruption")));
