...
| `inferadb_engine_auth_duration_seconds`             | Histogram | `method`, `tenant_id`               | Duration of authentication operations in seconds |
...
sum(rate(inferadb_engine_auth_success_total[5m])) by (tenant_id)
/
(sum(rate(inferadb_engine_auth_success_total[5m])) by (tenant_id) + sum(rate(inferadb_engine_auth_failure_total[5m])) by (tenant_id))
* 100
```
...
histogram_quantile(0.99,
  sum(rate(inferadb_engine_auth_duration_seconds_bucket[5m])) by (method, le)
) * 1000
```
...
sum(rate(inferadb_engine_auth_attempts_total[5m]))
```
        ...
        "panels": [
            {
                ...
                "targets": [
                    {
                        "expr": "sum(rate(inferadb_engine_auth_success_total[5m])) / (sum(rate(inferadb_engine_auth_success_total[5m])) + sum(rate(inferadb_engine_auth_failure_total[5m]))) * 100"
                    }
                ],
                "type": "gauge"
            },
            {
                ...
                "targets": [
                    {
                        "expr": "sum(rate(inferadb_engine_auth_attempts_total[5m]))",
                        "legendFormat": "Total"
                    },
                    {
                        "expr": "sum(rate(inferadb_engine_auth_attempts_total[5m])) by (method)",
                        "legendFormat": "{{method}}"
                    }
                ],
                "type": "graph"
            },
            {
                ...
                "targets": [
                    {
                        "expr": "histogram_quantile(0.50, sum(rate(inferadb_engine_auth_duration_seconds_bucket[5m])) by (le)) * 1000",
                        "legendFormat": "p50"
                    },
                    {
                        "expr": "histogram_quantile(0.95, sum(rate(inferadb_engine_auth_duration_seconds_bucket[5m])) by (le)) * 1000",
                        "legendFormat": "p95"
                    },
                    {
                        "expr": "histogram_quantile(0.99, sum(rate(inferadb_engine_auth_duration_seconds_bucket[5m])) by (le)) * 1000",
                        "legendFormat": "p99"
                    }
                ],
                "type": "graph"
            },
            {
                ...
                "targets": [
                    {
                        "expr": "sum(rate(inferadb_engine_auth_failure_total[5m])) by (error_type)",
        ...
        "panels": [
            {
                ...
                "targets": [
                    {
                        "expr": "sum(rate(inferadb_engine_auth_failure_total[5m]))",
                    ...
                    "conditions": [
                        {
                            ...
                            "query": {
                                "model": "sum(rate(inferadb_engine_auth_failure_total[1m]))"
                            }
                        }
                    ]
                }
            },
            {
                ...
                "targets": [
                    {
                        "expr": "topk(10, sum(rate(inferadb_engine_auth_failure_total[5m])) by (tenant_id))",
...
```rust
use inferadb_engine_observe::TracingConfig;
...
inferadb_engine_observe::init_tracing_with_config(config)?;
```
...
```rust
use inferadb_engine_observe::span_utils;
...
```rust
use inferadb_engine_auth::{AuditEvent, log_audit_event};
use chrono::Utc;
    ...
    - name: authentication_alerts
      rules:
          - alert: HighAuthenticationFailureRate
            expr: sum(rate(inferadb_engine_auth_failure_total[5m])) > 10
            for: 5m
            labels:
                severity: warning
            annotations:
          ...
          - alert: AuthenticationLatencyHigh
            expr: histogram_quantile(0.99, sum(rate(inferadb_engine_auth_duration_seconds_bucket[5m])) by (le)) > 1.0
            for: 5m
            labels:
                severity: warning
            annotations:
