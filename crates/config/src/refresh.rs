...
impl ConfigRefresher {
    ...
    async fn reload_config(&self, trigger: &str) {
        match crate::load(&self.config_path) {
            Ok(new_config) => {
    ...
    pub async fn refresh_once(&self) -> Result<bool, Box<dyn std::error::Error>> {
        ...
        let new_config = crate::load(&self.config_path)?;
...
"#,
        ...
        assert_eq!(current.listen.http, "127.0.0.1:9090", "Address should be updated");
    }
...
"#,
        ...
        // Refresh should fail validation (invalid backend)
        let result = refresher.refresh_once().await;
        assert!(result.is_err(), "Should reject invalid storage backend");
