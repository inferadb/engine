...
async fn test_write_relationships_in_different_vaults() {
    ...
    let key_alice = inferadb_engine_types::RelationshipKey {
        resource: "document:readme".to_string(),
        relation: "viewer".to_string(),
    ...
    let results_a = state
        .store
        .read(vault_a, &key_alice, inferadb_engine_types::Revision(u64::MAX))
        .await
        .unwrap();
    ...
    let key_bob = inferadb_engine_types::RelationshipKey {
        resource: "document:readme".to_string(),
        relation: "viewer".to_string(),
    ...
    let results_b = state
        .store
        .read(vault_b, &key_bob, inferadb_engine_types::Revision(u64::MAX))
        .await
        .unwrap();
    ...
    let results_a_bob = state
        .store
        .read(vault_a, &key_bob, inferadb_engine_types::Revision(u64::MAX))
        .await
        .unwrap();
    assert_eq!(results_a_bob.len(), 0);

    // Verify vault B doesn't have Alice
    let results_b_alice = state
        .store
        .read(vault_b, &key_alice, inferadb_engine_types::Revision(u64::MAX))
        .await
        .unwrap();
...
async fn test_operations_scope_to_correct_vault() {
    ...
    let all_a = state
        ...
        .list_relationships(
            ...
            Some("viewer"),
            None,
            inferadb_engine_types::Revision(u64::MAX),
        )
        .await
        .unwrap();
...
async fn test_vault_scoped_listing() {
    ...
    let resources_a = state
        ...
        .list_resources(
            vault_a,
            inferadb_engine_types::ListResourcesRequest {
                subject: "user:alice".to_string(),
    ...
    let resource_service_b = Arc::new(inferadb_engine_api::services::ResourceService::new(
...
async fn test_default_vault_fallback_when_auth_disabled() {
    ...
    store
        .create_organization(inferadb_engine_types::Organization {
            id: organization_a,
    ...
    store
        .create_vault(inferadb_engine_types::Vault {
            id: vault_a,
    ...
    let results =
        store.read(vault_a, &key, inferadb_engine_types::Revision(u64::MAX)).await.unwrap();
...
async fn test_organization_can_own_multiple_vaults() {
    ...
    let organization_obj = inferadb_engine_types::Organization {
        id: organization,
        name: "Test Organization".to_string(),
        created_at: chrono::Utc::now(),
        updated_at: chrono::Utc::now(),
    ...
    let vault_obj_1 = inferadb_engine_types::Vault {
        id: vault1,
        organization,
    ...
    let vault_obj_2 = inferadb_engine_types::Vault {
        id: vault2,
        organization,
    ...
    let vault_obj_3 = inferadb_engine_types::Vault {
        id: vault3,
        organization,
...
async fn test_read_from_wrong_vault_returns_empty() {
    ...
    let results =
        state.store.read(vault_b, &key, inferadb_engine_types::Revision(u64::MAX)).await.unwrap();
...
async fn test_cached_data_doesnt_leak_between_vaults() {
    ...
    let result_a = state
        ...
        .evaluate(
            vault_a,
            inferadb_engine_types::EvaluateRequest {
                subject: "user:alice".to_string(),
                resource: "document:cached".to_string(),
                permission: "viewer".to_string(),
                context: None,
    ...
    assert!(
        matches!(result_a, inferadb_engine_types::Decision::Allow),
    ...
    assert!(
        matches!(result_b, inferadb_engine_types::Decision::Deny),
