...
use infera_store::{InferaStore, MemoryBackend, RelationshipStore};
...
async fn test_relationships_written_to_vault_a_not_visible_in_vault_b() {
    ...
    let results_a = store.read(vault_a, &key_a, Revision(u64::MAX)).await.unwrap();
    ...
    let results_a_tries_b = store.read(vault_a, &key_b, Revision(u64::MAX)).await.unwrap();
    assert_eq!(results_a_tries_b.len(), 0, "Vault A should not see vault B's data");

    // Verify vault B can see its own data
    let results_b = store.read(vault_b, &key_b, Revision(u64::MAX)).await.unwrap();
    assert_eq!(results_b.len(), 1);

    // Verify vault B cannot see vault A's data
    let results_b_tries_a = store.read(vault_b, &key_a, Revision(u64::MAX)).await.unwrap();
...
async fn test_revision_tokens_are_vault_scoped() {
    ...
    let current_a = store.get_revision(vault_a).await.unwrap();
    let current_b = store.get_revision(vault_b).await.unwrap();
...
async fn test_delete_operations_only_affect_target_vault() {
    ...
    store.delete(vault_a, &key).await.unwrap();

    // Verify deleted from vault A
    let results_a = store.read(vault_a, &key, Revision(u64::MAX)).await.unwrap();
    assert_eq!(results_a.len(), 0, "Should be deleted from vault A");

    // Verify still exists in vault B
    let results_b = store.read(vault_b, &key, Revision(u64::MAX)).await.unwrap();
...
async fn test_filter_based_operations_scoped_to_vault() {
    ...
    let (_, deleted) = store.delete_by_filter(vault_a, &filter, None).await.unwrap();
...
async fn test_vault_isolation_under_load() {
    ...
    let handle_a = tokio::spawn(async move {
        for i in 0..200 {
            ...
            let results = store_a.read(vault_a, &key, Revision(u64::MAX)).await.unwrap();
    ...
    let handle_b = tokio::spawn(async move {
        for i in 0..200 {
            ...
            let results = store_b.read(vault_b, &key, Revision(u64::MAX)).await.unwrap();
            assert_eq!(results.len(), 1);
        }
    });
