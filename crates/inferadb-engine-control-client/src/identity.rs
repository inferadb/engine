...
impl ServerIdentity {
    ...
    fn generate_server_id() -> String {
        ...
        if let Ok(pod_name) = std::env::var("HOSTNAME") {
            // In Kubernetes, HOSTNAME is typically the pod name (e.g., "inferadb-engine-0")
            return format!("engine-{}", pod_name);
        }
        ...
        format!("engine-{}-{}", hostname, random_suffix)
    }
...
/// Thread-safe server identity
pub type SharedServerIdentity = Arc<ServerIdentity>;
...
mod tests {
    ...
    fn test_generate_server_identity() {
        ...
        // server_id should start with "engine-"
        assert!(identity.server_id.starts_with("engine-"));
