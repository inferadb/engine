...
use inferadb_engine_auth::middleware::optional_auth_middleware;
...
use inferadb_engine_auth::extractor::OptionalAuth;
use inferadb_engine_auth::middleware::require_scope;
...
```rust
use inferadb_engine_auth::extractor::RequireAuth;
...
```rust
use inferadb_engine_auth::middleware::require_any_scope;
...
```bash
cargo test -p inferadb-engine-auth
```
...
```bash
cargo test -p inferadb-engine-api --test auth_integration_tests
```
...
See the [integration tests](tests/auth_integration_tests.rs) in `inferadb-engine-api` for complete examples of:
