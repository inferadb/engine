...
impl IntrospectionClient {
    ...
    pub async fn introspect(
        ...
        if let Some(cache) = &self.cache {
            ...
            if let Some(cached) = cache.get(&cache_key).await {
                ...
                inferadb_engine_observe::metrics::record_oauth_introspection_cache_hit();
                return Ok(cached);
            }

            // Cache miss - perform introspection
            tracing::debug!("Introspection cache miss");
            inferadb_engine_observe::metrics::record_oauth_introspection_cache_miss();
    ...
    async fn introspect_uncached(
        ...
        inferadb_engine_observe::metrics::record_oauth_introspection(success, duration);
...
pub async fn validate_oauth_jwt(
    ...
    // Record metrics
    let success = result.is_ok();
    inferadb_engine_observe::metrics::record_oauth_jwt_validation(&issuer, success);
