...
//! ## Architecture
//!
//! The JWKS cache fetches public keys from a Control Plane API endpoint at
//! `{base_url}/control/v1/organizations/{org_id}/jwks.json`. Keys are cached using a Moka cache
...
impl JwksCache {
    ...
    pub async fn get_jwks(&self, org_id: &str) -> Result<Vec<Jwk>, AuthError> {
        ...
        if let Some(cached) = self.cache.get(&key).await {
            if cached.fetched_at.elapsed() < self.ttl {
                ...
                inferadb_engine_observe::metrics::record_jwks_cache_hit(org_id);
        ...
        inferadb_engine_observe::metrics::record_jwks_cache_miss(org_id);
    ...
    async fn fetch_jwks(
        ...
        let url = format!("{}/control/v1/organizations/{}/jwks.json", base_url, org_id);
        ...
        inferadb_engine_observe::metrics::record_jwks_refresh(org_id, duration, success);
