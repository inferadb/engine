...
use inferadb_engine_types::{Relationship, Revision};
...
async fn test_conflict_resolution_last_write_wins() {
    ...
    assert!(matches!(resolution, inferadb_engine_repl::Resolution::KeepRemote));
}
...
async fn test_conflict_resolution_source_priority() {
    ...
    local.metadata = Some(inferadb_engine_repl::ChangeMetadata {
    ...
    remote.metadata = Some(inferadb_engine_repl::ChangeMetadata {
...
async fn test_conflict_resolution_insert_wins() {
    ...
    assert!(matches!(resolution, inferadb_engine_repl::Resolution::KeepLocal));
}
    ...
    for next in &changes[1..] {
        if resolver.detect_conflict(&current, next) {
            ...
            current = match resolution {
                inferadb_engine_repl::Resolution::KeepLocal => current,
                inferadb_engine_repl::Resolution::KeepRemote => next.clone(),
...
async fn test_topology_validation_no_primary() {
    ...
    topology.add_region(inferadb_engine_repl::Region::new(
        RegionId::new("us-west-1"),
        "US West 1".to_string(),
        false, // not primary!
    ));
    ...
    assert!(matches!(result.unwrap_err(), inferadb_engine_repl::TopologyError::NoPrimaryRegion));
}
...
async fn test_topology_validation_multiple_primary() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        inferadb_engine_repl::TopologyError::MultiplePrimaryRegions
    ));
}
    ...
    if let Some(region) = topology.get_region_mut(&RegionId::new("eu-central-1")) {
        if let Some(zone) = region.zones.get_mut(0) {
            if let Some(node) = zone.nodes.get_mut(0) {
                node.status = inferadb_engine_repl::NodeStatus::Unreachable;
            }
        }
    }
...
async fn test_concurrent_conflict_resolution() {
    ...
    for handle in handles {
        ...
        assert!(matches!(
            result,
            inferadb_engine_repl::Resolution::KeepLocal
                | inferadb_engine_repl::Resolution::KeepRemote
        ));
    }
}
...
async fn test_multi_region_write_propagation() {
    ...
    change.metadata = Some(inferadb_engine_repl::ChangeMetadata {
...
async fn test_cross_region_conflict_with_metadata() {
    ...
    us_change.metadata = Some(inferadb_engine_repl::ChangeMetadata {
        source_node: Some("us-west".to_string()),
        causality_token: None,
        tags: std::collections::HashMap::new(),
    });
    ...
    eu_change.metadata = Some(inferadb_engine_repl::ChangeMetadata {
