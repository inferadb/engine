...
impl ConflictResolver {
    ...
    pub fn resolve(&self, conflict: &Conflict) -> Result<Resolution, ReplError> {
        ...
        match &resolution {
            Ok(Resolution::KeepLocal) => {
                inferadb_engine_observe::metrics::record_replication_conflict("local");
            },
            Ok(Resolution::KeepRemote) => {
                inferadb_engine_observe::metrics::record_replication_conflict("remote");
            },
            Ok(Resolution::KeepBoth) | Ok(Resolution::Merge(_)) => {
                inferadb_engine_observe::metrics::record_replication_conflict("merged");
            },
            Err(_) => {
                inferadb_engine_observe::metrics::record_replication_failure(1);
            },
        }
