...
echo "  - inferadb-api       → API layer (REST + gRPC)"
echo "  - inferadb-auth      → Authentication & authorization"
echo "  - inferadb-cache     → Result caching"
echo "  - inferadb-config    → Configuration management"
echo "  - inferadb-core      → Policy evaluation engine"
echo "  - inferadb-observe   → Observability (metrics, tracing, logging)"
echo "  - inferadb-repl      → Replication & consistency"
echo "  - inferadb-store     → Storage abstraction"
echo "  - inferadb-wasm      → WASM policy modules"
echo ""
